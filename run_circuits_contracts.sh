...
do
    starkli class-by-hash $contract_class --rpc https://mainnet.juno.internal.lambdaclass.com > contracts/{$contract_class}.contract_class.json
    jq 'del(.abi)' contracts/{$contract_class}.contract_class.json > contracts/{$contract_class}.json
    cargo run --bin starknet-native-compile contracts/{$contract_class}.json contracts/{$contract_class}_out.dylib --stats contracts/{$contract_class}_stats.json
    sleep 2
done
