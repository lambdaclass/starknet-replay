...
- Saves output to bench_data/<tx>-<net> directory
- Prints speedup
EOF
}
...
$NATIVE_TARGET bench-tx "$TX" "$NET" "$BLOCK" "$LAPS" --output "$native_data_output" > "$native_log_output"
...
$VM_TARGET bench-tx "$TX" "$NET" "$BLOCK" "$LAPS" --output "$vm_data_output" > "$vm_log_output"
